...
// Employee IDs
const employeeIds: number[] = [101, 102, 103, 104, 105];

// forEach()
console.log("Employee IDs:");
employeeIds.forEach(function (id: number): void {
...
const loginAttempts: number[] = [1, 2, 3, 4, 5];
...
// Set
const activeProjects: Set<string> = new Set(["Payroll", "CRM", "ERP", "Support"]);
...
// Map
const employeeStatus: Map<string, string> =
...
// while
let pendingTickets: number = 1;
while (pendingTickets <= 5) {
    console.log(
        `Resolving Ticket #${pendingTickets}`
    );
    pendingTickets++;
}
...
do {
    console.log(
        `Attempting API Connection - Try ${retryCount}`);
    retryCount++;
} while (retryCount <= 3);
